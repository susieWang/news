...
export default class InfiniteScroll extends Component {
  ...
  onMove(evt) {
    ...
    let {parentId,scrollPositionH} = this.props;
    var scrollTop = 0;
    if(parentId) scrollTop = document.getElementById(parentId).scrollTop;
    console.log(scrollTop);
    if (this.currentY < this.startY) {
      if((scrollPositionH+scrollTop+50) > this._infScroll.offsetHeight){
        this.setState({ showLoader: true });
      }
      return 
    };
    if (this.currentY - this.startY >= this.props.pullDownToRefreshThreshold  && scrollTop < 20) {
      this.setState({
  ...
  onEnd(evt) {
   ...
   //document.body.scrollTop
    // console.log(document.getElementById("bodyContainer").scrollTop);
    // console.log(this.startY + "," +this.currentY);
    this.startY = 0;
    this.currentY = 0;
